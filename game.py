...
class WordGuess:
    ...
    def __str__(self) -> str:
        ...
        for letter, state in zip(self.guess_letters, self.guess_state):
            guess_str += (
                f"[{self.letter_colours[int(state)]}]{letter.upper()}[/{self.letter_colours[int(state)]}] "
            )
        return guess_str
...
class WordleMatch:
    ...
    def __init__(self, max_guesses: int, word_list: Set[str], target_word: Optional[str] = None) -> None:
    ...
    def is_game_over(self) -> bool:
        ...
        if self.guess_list and self.guess_list[-1].is_all_correct():
            return True
